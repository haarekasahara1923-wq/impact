...
export default async function MediaPage() {
  ...
  try {
    toppers = await sql('SELECT * FROM toppers ORDER BY id ASC');
    galleryItems = await sql('SELECT * FROM gallery_items ORDER BY id DESC');
  } catch (error) {
    console.error('Failed to fetch media data from database:', error);
    dbError = true;
  }
    ...
    <div className="bg-white min-h-screen pb-20">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-16">
        ...
        <section className="mb-20">
          ...
          {toppers.length === 0 && !dbError ? (
            <p className="text-center text-slate-500 py-10">No toppers have been added yet. Add them from the Admin Panel.</p>
          ) : (
        ...
        <section className="border-t border-slate-100 pt-16">
          ...
          {galleryItems.length === 0 && !dbError ? (
            <p className="text-center text-slate-500 py-10">No gallery items have been added yet. Add them from the Admin Panel.</p>
          ) : (
